...
function inv(A::SymDPR1{T}, σ::Float64, τ::Float64=1.0e3) where T
    ...
    if Kρ < τ
        ...
        # JJH 4-21-20 BEGIN
        Qd=convert(Float64,Qd)
        Pd=convert(Float64,Pd)
        # JJH 4-21-20 END
...
function inv(A::SymDPR1{T}, σ::Double) where T
    ...
    n=length(A.D)
    ## JJH 4-22-20 Begin
    D=Array{Double,1}(undef,n)#Array(Double,n)
    u=Array{Double,1}(undef,n)#Array(Double,n)
    ## JJH 4-22-20 END
    for k=1:n
        D[k]=Double(A.D[k])-σ
    end
    ...
    ## JJH 4-22-20 BEGIN
    D1=Array{T,1}(undef,n)#Array{T}(n)
    u1=Array{T,1}(undef,n)#Array{T}(n)
    ## JJH 4-22-20 END
    for k=1:n
        D1[k]=D[k].hi+D[k].lo
        u1[k]=u[k].hi+u[k].lo
    ...
    if abs(ν)==Inf
        ...
        while Kν>τ[3]
            ...
            if count(isequal(σ₁),A.D)>0 # we came back with a pole
                ...
                ## JJH 4-22-20 BEGIN
                #display(length(A.D))
                D0=[convert(Double,A.D[i]-σᵥ) for i in 1:length(A.D)]#map(A.D,Double)-σᵥ
                D1=[D0[i].hi+D0[i].lo for i in 1:length(D0)] #D1=D0.hi+D0.lo
                ## JJH 4-22-20 END
...
function eigen(A::SymDPR1{T}, τ::Vector{Float64}=[1e3,10.0*length(A.D),1e3,1e3,1e3]) where T
    ...
    if !isempty(g0)
        ...
        lg0=length(g0)
        ## JJH 4-22-20 BEGIN
        R=Array{Tuple{LinearAlgebra.Givens{Float64},Float64},1}(undef,lg0)#Array(Tuple{LinearAlgebra.Givens{Float64},Float64},lg0)
        ## JJH 4-22-20 END
        for l=lg0:-1:1
