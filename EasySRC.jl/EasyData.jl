...
function retrieveDataset(dsetname::String)
    ...
    if dsetname == "TREX13"
        ...
        Z2  = CSV.read("G:/Code/ILKE_experiments_julia/TREX13Data.csv"; header=false)
        y2  = CSV.read("G:/Code/ILKE_experiments_julia/TREX13Labels.csv"; header=false)
        Y       = convert(Array{Float64,2},Z2)
        labels  = vec(convert(Array,y2))
    elseif dsetname=="FRM"
        Y       = CSV.read("G:/Code/ILKE_experiments_julia/FRMData.csv"; header=false)
        labels  = CSV.read("G:/Code/ILKE_experiments_julia/FRMLabels.csv"; header=false)
        Y       = convert(Array{Float64,2},Y)
        labels  = vec(convert(Array,labels))
    elseif dsetname == "BAYEX14"
        #Open Dataset
        Z3  = CSV.read("G:/Code/ILKE_experiments_julia/BAYEX14Data.csv"; header=false)
        y3  = CSV.read("G:/Code/ILKE_experiments_julia/BAYEX14Labels.csv"; header=false)
        Y       = convert(Array{Float64,2},Z3)
        labels  = vec(convert(Array,y3))
    ...
    elseif dsetname == "USPS"
        Z3  = CSV.read("G:/Code/ILKE_experiments_julia/backup/USPSData.csv"; header=false)
        y3  = CSV.read("G:/Code/ILKE_experiments_julia/backup/USPSLabels.csv"; header=false)
        Y= convert(Array{Float64,2},Z3)
        labels= vec(convert(Array,y3))
